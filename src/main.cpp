...
#define AUDIO_OPUS_FRAME_MS   5   // one of 2.5, 5, 10, 20, 40, 60, 80, 100, 120 init 40
#define AUDIO_OPUS_BITRATE    64000// bit rate from 2400 to 512000
...
/*
  if (radio.available() > 0) {
    ...
    if (xSemaphoreTake(receiveSemaphore, 0) == pdTRUE) {
      ...
      xTaskCreatePinnedToCore(
        [](void *){
          while (radio.readData(dataIn, radio.getPacketLength()) == RADIOLIB_ERR_NONE) {
            onReceive();
          }
          xSemaphoreGive(receiveSemaphore);
        },
...
void onReceive() {
  ...
  int test = opus_decode(opus_decoder_, dataIn, test, uncompressed, OPUS_FRAME_SIZE, 0);
